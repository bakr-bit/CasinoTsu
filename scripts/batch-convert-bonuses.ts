/**
 * Batch Convert Bonuses Script
 * Converts all bonus type guide articles from markdown to MDX + extracts structured bonus type data
 *
 * Bonus types are category guides (welcome-bonus, no-deposit, cashback, etc.)
 * that list casinos offering each bonus type.
 *
 * Usage:
 *   npx tsx scripts/batch-convert-bonuses.ts              # Convert all
 *   npx tsx scripts/batch-convert-bonuses.ts --skip-existing  # Skip already converted
 *   npx tsx scripts/batch-convert-bonuses.ts --only=welcome-bonus,no-deposit  # Convert specific slugs
 *   npx tsx scripts/batch-convert-bonuses.ts --dry-run    # List what would be converted
 */
...
const NEW_MDX_DIR = path.join(process.cwd(), 'content', 'mdx', 'bonuses');
...
function buildMDXSystemPrompt(slug: string): string {
  return `You are converting a Japanese bonus guide article from Markdown to MDX format.
...
- Example: Instead of a table showing bonus amounts, write: "ベラジョンは初回100%最大11万円..."
...
Use CasinoList to show casinos offering this bonus type:
\`\`\`jsx
<CasinoList toplistId="${slug}-casinos" limit={5} title="[Bonus Type Name]おすすめカジノ" />
\`\`\`
...
- If the source content is incomplete, you MUST write a conclusion that summarizes the bonus type's key points
...
const BONUS_TYPE_SCHEMA = {
  type: 'OBJECT',
  properties: {
    name: {
      type: 'STRING',
      description: 'Bonus type name in English (e.g., "Welcome Bonus", "No Deposit Bonus")',
    },
    nameJa: {
      type: 'STRING',
      description: 'Bonus type name in Japanese (e.g., "ウェルカムボーナス", "入金不要ボーナス")',
    },
    description: {
      type: 'STRING',
      description: 'Brief description of this bonus type in Japanese (1-2 sentences)',
    },
    highlights: {
      type: 'ARRAY',
      items: { type: 'STRING' },
      description: 'List of 3-5 key advantages/highlights of this bonus type in Japanese',
    },
    watchouts: {
      type: 'ARRAY',
      items: { type: 'STRING' },
    ...
    recommendedCasinos: {
      ...
      items: { type: 'STRING' },
      description:
        'List of 3-5 recommended casino slugs mentioned in the article (e.g., ["vera-john", "stake", "bons"]). Use kebab-case slugs.',
    },
    metaTitle: {
      type: 'STRING',
      description: 'SEO title in Japanese with year (e.g., "入金不要ボーナス完全ガイド【2025年最新版】")',
    },
    metaDescription: {
      type: 'STRING',
  ...
  required: ['name', 'nameJa', 'description', 'highlights', 'recommendedCasinos', 'metaTitle', 'metaDescription'],
};

interface ExtractedBonusTypeData {
  name: string;
  nameJa: string;
  description: string;
  highlights: string[];
...
async function callGeminiStructured(prompt: string): Promise<ExtractedBonusTypeData> {
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${GEMINI_API_KEY}`,
    {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseSchema: BONUS_TYPE_SCHEMA,
        },
      }),
    }
  );
  ...
  return JSON.parse(jsonText) as ExtractedBonusTypeData;
}
...
async function extractBonusTypeData(slug: string, markdown: string): Promise<ExtractedBonusTypeData> {
  const prompt = `Extract structured bonus type data from this Japanese bonus guide article.
...
1. **name**: The bonus type's English name (e.g., "Welcome Bonus", "No Deposit Bonus", "Cashback")
2. **nameJa**: The bonus type's Japanese name (e.g., "ウェルカムボーナス", "入金不要ボーナス", "キャッシュバック")
3. **description**: Brief description in Japanese (1-2 sentences)
4. **highlights**: 3-5 key advantages of this bonus type in Japanese
5. **watchouts**: 1-3 potential concerns in Japanese (optional but recommended)
6. **recommendedCasinos**: 3-5 casino slugs mentioned as recommended in the article
   - Use kebab-case slugs (e.g., "vera-john", "stake", "mega-dice")
   - Extract from the rankings/recommendations in the article
   - Common casino name to slug mappings:
     ...
     - ミスティーノ → mystino
     - Mistyino → mystino
7. **metaTitle**: SEO title (include bonus type name and year)
8. **metaDescription**: SEO description (120-160 characters, Japanese)
...
function generateTypeScriptFile(slug: string, data: ExtractedBonusTypeData): string {
  ...
  return `import type { BonusTypeData } from './types';

export const ${varName}: BonusTypeData = {
  slug: '${slug}',
...
async function convertSingleBonusType(slug: string): Promise<ConversionResult> {
  ...
  try {
    ...
    const sourcePath = path.join(OLD_CONTENT_DIR, 'bonuses', slug, 'rewritten.md');
    ...
    // Extract bonus type data
    console.log(`  [${slug}] Extracting bonus type data...`);
    const bonusData = await extractBonusTypeData(slug, markdown);
    const tsContent = generateTypeScriptFile(slug, bonusData);
    await fs.writeFile(path.join(BONUS_DATA_DIR, `${slug}.ts`), tsContent);
...
async function processBatch(slugs: string[]): Promise<ConversionResult[]> {
  const results = await Promise.all(slugs.map((slug) => convertSingleBonusType(slug)));
  return results;
}

async function getAllSlugs(): Promise<string[]> {
  const entries = await fs.readdir(path.join(OLD_CONTENT_DIR, 'bonuses'), { withFileTypes: true });
  return entries
...
async function main(): Promise<void> {
  ...
  await fs.mkdir(BONUS_DATA_DIR, { recursive: true });
  ...
  if (skipExisting) {
    ...
    console.log(`Skipping ${existing.size} already converted bonus types`);
  }

  console.log(`\n📋 Bonus types to convert: ${slugs.length}`);
  ...
  const logPath = path.join(process.cwd(), 'bonus-conversion-log.json');
  await fs.writeFile(
    logPath,
    JSON.stringify(
      {
