...
export const metadata: Metadata = {
  title: "Justspin（ジャストスピン）の入金不要ボーナスや評判について",
  description: "21.comの姉妹サイト、ジャストスピンの入金不要ボーナス、ウェルカムパッケージ、ゲームラインナップ、日本語サポートの評判をCasinoTsuが徹底レビューします。",
};

export default async function JustSpinReviewPage() {
  const { content, frontmatter } = await loadMDX('reviews', 'just-spin');
  const casinoData = getCasino('just-spin');
